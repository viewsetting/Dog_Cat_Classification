...
def train_merge(epoch=1,batch_size=16,optimizer="adam",lr=1e-4,loss_func="BCE",val_ratio=0.2,model_save_path='./tmp/merge/',log_path='./log/merge',freeze=False):
    ...
    #get_dataloader
    train_dataloader,val_dataloader = get_data_loader(batch_size=batch_size,mode='train',val_ratio=val_ratio,transformList=["resnet","inception","resnext"])
...
def test_merge(model_path='./tmp/merge/epoch_num_2.pth',batch_size=64,csv_path="./results/Merge_prob.csv",binary=False):
    ...
    #get_dataloader
    test_dataloader = get_data_loader(batch_size=batch_size,mode='test',dir=TEST_DIR,transformList=["resnet","inception","resnext"])
